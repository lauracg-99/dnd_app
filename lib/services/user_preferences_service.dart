  ...
  static Future<void> initializeStorage() async {
    try {
      if (kIsWeb) {
        ...
        debugPrint(
          'Web platform detected, using memory-only storage for preferences',
        );
        return;
      }
      ...
      final testFile = File(
        '${testDir.path}/test_prefs_${DateTime.now().millisecondsSinceEpoch}.tmp',
      );
      ...
      try {
        ...
        debugPrint(
          'File system operations available, using file storage for preferences',
        );
      } catch (e) {
        _useMemoryStorage = true;
        debugPrint(
          'File system operations failed, using memory storage for preferences: $e',
        );
      }
    } catch (e) {
      _useMemoryStorage = true;
      debugPrint(
        'Storage initialization failed, using memory storage for preferences: $e',
      );
    }
  }
...
class UserPreferences {
  ...
  factory UserPreferences.fromJson(Map<String, dynamic> json) {
    final rawOrder = List<String>.from(
      json['characterTabOrder'] ??
          UserPreferences.defaultPreferences().characterTabOrder,
    );

    return UserPreferences(
      characterTabOrder:
          rawOrder
              .map((tabId) => tabId == 'familiar' ? 'companion' : tabId)
              .toList(),
